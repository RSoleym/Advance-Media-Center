...
void TIMER0_IRQHandler(void) 
{
  ...
  uint32_t cnt;
	bool check = true;
	while(check){
		dir = get_button();							  /*Recieves the joystick input*/	
		switch(dir){												
			case KBD_SELECT:
					NVIC_SystemReset();                     /*Resets the board*/
				check = false;
				break;
			default:
...
int audio_main (void)
{
  ...
  GLCD_DisplayString(0, 0, 1, (unsigned char*) "    COE718 Project  ");
	GLCD_DisplayString(1, 0, 1, (unsigned char*) "  Ramtin Soleymani   ");
	GLCD_SetTextColor(White);
  GLCD_DisplayString(2, 0, 1, (unsigned char*) "       Main Menu     ");
